...
| 9 | Both roles default to `deepseek/deepseek-v4-flash-0731`, kept as two separate settings | One `MODEL` setting; or a deliberately cheaper extractor | Verified served against the live catalog. The roles want different things — prose quality vs. valid JSON — so keeping two names costs nothing and lets either move independently later |
